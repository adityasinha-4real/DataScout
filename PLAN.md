...
- **Frontend:** React 19.3 + TypeScript 5.9, built by Vite 8.3 (`@vitejs/plugin-react` 6.1). Router: `react-router-dom` 7.18. State: React context (`src/lib/auth.tsx`) — no external state library, no token storage (session is an HttpOnly cookie; `/me` restores it). API base URL: `import.meta.env.VITE_API_BASE_URL`, read only in `src/lib/api.ts`, blank by default → relative `/api` (Vite proxy in dev/preview, `vercel.mjs` rewrite in production), no literal fallback.
- **Backend:** Node 24.13 (ESM), Express 5.2. Entry `backend/src/server.js` (runs `assertStartable`: production needs TRUST_PROXY); app factory `backend/src/app.js` with injectable `deps` (`llm`, `now`, `rateLimitStore`). Default port 4000. Routes: `src/routes/health.js`, `auth.js`, `datasets.js`, `debug.js` (only with `DEBUG_IP_ENDPOINT=1`). Middleware: `cors` (only when `CORS_ORIGIN` is non-empty), `express.json`, `express.text` (CSV), the sign-in rate limiter, `requireAuth` from `src/auth/sessions.js`, body-error normaliser, `errorHandler`. 16 routes, audited in §9.
- **Database:** SQLite through Node's built-in `node:sqlite` (`DatabaseSync`) — no external service, no native build. No ODM; hand-written SQL in `src/db/index.js`. Tables: `users` (incl. `token_version`, `revoked_sessions`, added to old databases by `applySchema`), `datasets`. Connection env var: `DATABASE_URL` (`:memory:` under test).
- **Auth:** scrypt password digests via `node:crypto` (`src/auth/password.js`); HS256 JWTs signed and verified in `src/auth/jwt.js` with the algorithm fixed rather than read from the header. Issued by `/api/auth/register` and `/api/auth/login` with claims `sub`, `ver` (token_version) and `sid` (session id), 900 s TTL; carried in the `datascout_session` cookie (or Bearer). `src/auth/sessions.js` verifies, slides cookie sessions past half-life, and revokes: `/logout` one sid, `/logout-all` every token via `token_version`.
...
- **Scripts available:** `backend`: `start`, `dev`, `test`, `lint`. `frontend`: `dev`, `build`, `preview`, `test`, `lint`. `e2e`: `test`, `install-browsers`, `lint`. Repo: `scripts/verify.sh` (the gate, frozen), `scripts/goal-check.sh` (guard-file diff with the one approved edit, plus the placeholder check).
- **Test setup:** `node --test` with built-in coverage (backend: 81 tests in 6 files at baseline → 189 in 14 files now, 98.07% lines); Playwright 1.57 (e2e: 4 → 8); Vitest 5 + Testing Library in jsdom (frontend: 20 tests in 5 files, incl. `vercel.test.mjs`). No tests existed before this run.
...
- [Logout semantics] DONE — commit 5f7c03d — tokens carry `sid` (set at sign-in, kept by sliding re-issue). `POST /api/auth/logout` revokes this session only: `{sid, until: now + TTL}` appended to `users.revoked_sessions` with expired entries pruned on the same write; `requireAuth` refuses a revoked sid. `POST /api/auth/logout-all` takes over the old behaviour (bump `token_version`, clear the list). Only a currently valid token triggers either. `backend/test/revocation.test.js` rewritten (13 tests): logout on A leaves B working; a copied pre-logout token is 401 on `/me` and `/datasets`; a slid re-issue and its predecessor both die with the session; re-login starts a new sid; pruning (A dropped at t=900 on the next write, B kept, `until` = now+900); logout-all kills A and B and empties the list; other users untouched; a replayed revoked token at `/logout` or `/logout-all` changes nothing; a pre-logout-all token can't trigger another; no/garbage credentials revoke nothing; missing/invalid `ver` or `sid` → 401; old-schema migration. Mutations, each restored and hash-verified: revoked-sid check removed → 5 fail; version check removed → 3 fail (the logout-all ones); pruning removed → 1 fails (the prune test); sign-out trusting an unverified token → 7 fail (incl. both replay tests). Backend 173 tests, 99.01% lines; e2e 8 passed. Supersedes the "logout ends every session" entry above.
...
- [Placeholder grep] DONE — commit 0f9adea — the bare-prefix grep is replaced by an anchored pattern (in the goal-check block above and inside `scripts/goal-check.sh`): it matches only progress-log entry lines carrying the template's angle-bracket sha placeholder (the format in the §9 comment) or an uppercase SHA-underscore token (the form the owner's prompt named). The earlier `SHA[_]` rewording is reverted to a plain description. Shown in the transcript: the real PLAN.md → no match; a copy with four planted lines → exactly the two real placeholders reported, the prose mention and a real `commits d9f32a4..a27a04e` range ignored; the template comment on §9 is ignored; `goal-check.sh` against the planted copy → FAIL, exit 1.

- [Rate-limit store] DONE — commit 45511f6 — `src/auth/rateLimit.js` defines the store interface (one async, atomic `hit(key, nowMs, windowMs, limit) → { allowed, oldestMs }`, documented as mappable to one Redis Lua script) and `createMemoryStore()` as the default; the limiter middleware is async and takes `store`; `createApp` accepts `deps.rateLimitStore`. Still in-memory, no Redis. New tests: the memory store alone (allow, deny without recording, slot freed after the window, keys independent); an injected recording store gets key/clock/60 s/limit=7 and its denial becomes 429 with `Retry-After: 45`; `/api/health` never consults it; a throwing store → 500 `INTERNAL_ERROR`, rest of the app fine; sweep test now reads `store.size()` (51 → 1). Mutation (injected store ignored) fails the 2 injection tests. Backend 189 tests, 98.07% lines. README documents the interface and that the in-memory store means one API instance (counters reset on restart, per-replica budgets).

- [Close-out] DONE — README rewritten for a developer with no other context (commit 162c059): env table for all 17 variables the code reads (reader, default, production requirement), Authentication section (cookie, same-origin, 900 s TTL + sliding re-issue, logout vs logout-all, CSRF), rate-limit limits, TRUST_PROXY measurement, troubleshooting. The README Quick start was then followed verbatim in a fresh clone (`cp .env.example .env`, fill `JWT_SECRET` with the README's generator, three `npm ci`, `npm run start --prefix backend`, `npm run dev --prefix frontend`): `:4000/api/health` and `:5173/api/health` (via the Vite proxy) both `{"status":"ok","db":"connected"}`, register through `:5173` → 201 with `Max-Age=900; HttpOnly; SameSite=Lax`. That run exposed `backend/data/datascout.db` as untracked; `.gitignore` fixed to `**/data/*.db` (commit 6551242), fresh clone then clean. §3 refreshed to the current code.
- [Scope E] DONE (re-verified on the final image) — built at 6551242 (image `sha256:55bf00cfbed0`): with no `TRUST_PROXY` the container prints "Configuration error: TRUST_PROXY must be set in production. Measure …" and exits 1; image env has `NODE_ENV=production`, `DATABASE_URL=/data/datascout.db`, `JWT_EXPIRES_IN=900`, no TRUST_PROXY. With `-e TRUST_PROXY=0` on fresh volume `datascout-verify-1790422320`: health ok; `/api/_debug/ip` 404; register 201 `Max-Age=900; HttpOnly; SameSite=Lax; Secure`; upload 201; after `docker restart` both sessions 200 and data intact; logout A → A 401, B 200; logout-all via B → B 401; re-login 200 with data; `uid=1000(node)`, `/data` `node:node 755`, db `node:node 644`; `docker stop` 842 ms, `ExitCode=0`, only `signal=15`.
- [Ledger] Every earlier open marker, resolved or carried forward:
  - AC-T5 OWNER-PENDING (min/max in prompt) → resolved: owner option B, documented (d4bb0b5).
  - Scope C OWNER-PENDING (×2) → resolved: deferred to iteration 3 (§11).
  - AC-T6 OWNER-PENDING (no revocation on logout) → resolved: a056359, refined to per-session + logout-all in 5f7c03d.
  - Scope E NOTE "image not built" → resolved: real builds verified three times, last on the final image (above).
  - TRUST_PROXY DONE at a787613 (`TRUST_PROXY=1` in the image) → superseded by the fail-safe in 1d175b2.
  - Revocation OWNER-PENDING (3600 s code default) → resolved: 900 in code (2c85825) under the owner's approval for app.test.js:87.
  - Same-origin "Vercel build not verified" → still unverified (no deploy); see §11.
  - TRUST_PROXY measurement BLOCKED → still BLOCKED; see §11.
  - **BLOCKED (owner merge)** — `scripts/verify.sh`'s guard diffs against `main`, which does not yet carry the approved app.test.js:87 line, so `bash scripts/verify.sh` stops at `[GUARD] FAIL … backend/test/app.test.js | 2 +-` / `1 CHECKS FAILED`. Unblock: merge branch `guard/ttl-default` (commit 249797a: config default + that one line, main's suite 108/108) into `main`; its app.test.js blob `f1300d8` equals this branch's, so the guard then passes. The same gate run against `origin/guard/ttl-default` (main + only that line, via verify.sh's own `VERIFY_BASE_REF`) reports 21 passed.

**Iteration 2 status:** every §5 criterion `[x]`; every item of the three follow-ups DONE except: TRUST_PROXY measurement (BLOCKED: no API host), live Vercel deploy of `vercel.mjs` (unverified: nothing deployed), and the final `verify.sh` guard (BLOCKED until the owner merges `guard/ttl-default`). Scope C is iteration-3 backlog.
...
- **Measure TRUST_PROXY on the real deployment** — follow README "Measuring TRUST_PROXY"; record the chain output and the chosen N here. Blocked on an API host.
- **Deploy-verify `frontend/vercel.mjs`** — a Vercel preview with `API_ORIGIN` set, confirming the `/api` rewrite, the SPA fallback, and the build failing without `API_ORIGIN`. Only unit-tested so far.
- **Shared rate-limit store** — needed before running more than one API instance (the `hit()` interface is ready; Redis not added), together with a networked database.
- **"Sign out everywhere" in the UI** — `POST /api/auth/logout-all` exists and is tested, but the frontend only offers per-session sign-out.
- **CSRF token or `__Host-` cookie** — only if untrusted subdomains ever share the app's registrable domain (see the §9 CSRF audit residual risk).

...
- **ML / data layer:** The analysis is deterministic code, not a model: type inference and statistics in `src/csv/profile.js`, query/rank engine in `src/csv/query.js`. A model is used for one thing only — turning a plain-English question into a `QuerySpec` (`src/llm/`), which is validated against a JSON schema and this dataset's real columns before the same deterministic engine runs it. Nothing the model returns is executed, and it never sees a cell value.
